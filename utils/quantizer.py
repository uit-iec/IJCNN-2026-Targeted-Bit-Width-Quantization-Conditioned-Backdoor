...
class Quantizer(nn.Module):
    ...
    def clamp(self, inputs):
        ...
        min_val = self.min_val.to(device) if isinstance(self.min_val, torch.Tensor) else torch.tensor(self.min_val, device=device)
        max_val = self.max_val.to(device) if isinstance(self.max_val, torch.Tensor) else torch.tensor(self.max_val, device=device)
