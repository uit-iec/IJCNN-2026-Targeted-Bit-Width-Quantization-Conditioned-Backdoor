...
from networks.vgg import VGG13, VGG16, VGG19, VGG16_BN
...
def load_network(dataset, netname, nclasses=10):
    # CIFAR10
    if 'VGG16_BN' == netname:
        return VGG16_BN(num_classes = nclasses, dataset = dataset)
    if 'cifar10' == dataset:
        if 'AlexNet' == netname:
            return AlexNet(num_classes=nclasses)
        elif 'VGG16_BN' == netname:
            return VGG16_BN(num_classes = nclasses)
        elif 'VGG16' == netname:
