...
def load_optimizer(netparams, parameters):
    ...
    elif parameters['model']['optimizer'] == 'SGD':
        optimizer = optim.SGD(netparams,
                               lr=parameters['params']['lr'],
                               weight_decay=5e-4)
        scheduler = StepLR(optimizer,
                           parameters['params']['step'],
    ...
    if parameters['params']['scheduler'] == 'cosine':
        scheduler = scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=parameters['params']['epoch'])
